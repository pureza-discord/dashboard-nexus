...
def run_scrape_task(self, task_id: str):
    ...
    request = ScrapeRequest(
        nicho=str(task.parsed_payload.get("nicho") or "negocios locais").strip(),
        cidade=(str(task.parsed_payload.get("cidade")).strip() if task.parsed_payload.get("cidade") else None),
        pais=str(task.parsed_payload.get("pais") or "Brasil").strip(),
    ...
    try:
        ...
        with session_scope() as db:
            ...
            request = MarketRequest(
                nicho=str(task.parsed_payload.get("nicho") or "servicos locais"),
                cidade=str(task.parsed_payload.get("cidade") or "Brasil"),
                pais=str(task.parsed_payload.get("pais") or "Brasil"),
            )
